...
export default function TripDetailsPage() {
  ...
  const isValidLink = useMemo(() => {
    ...
    // Usually there's only one OTP link per trip, but checking all anyway
    return data.shipmentOtps.some((otp: any) => {
      const url = otp.link; // "http://localhost:3000/verify-OTP/qlxjzeia-TRIP1"
      const path = new URL(url).pathname; // "/verify-OTP/qlxjzeia-TRIP1"
      const slug = path.split("/").pop(); // "qlxjzeia-TRIP1"
